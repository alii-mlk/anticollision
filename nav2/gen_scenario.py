...
def scenario_yaml(seed, start, goal, obstacles):
    sx, sy = start
    gx, gy = goal
    lines = [
...
def main():
    ...
    ap.add_argument("--out", type=Path, default=None,
                    help="output directory (default: scenarios/s<seed>_n<N>)")
    args = ap.parse_args()

    out = args.out or Path(__file__).parent / "scenarios" / f"s{args.seed}_n{args.n_obstacles}"
    out.mkdir(parents=True, exist_ok=True)

    rng = random.Random(args.seed)
    start, goal, obstacles = generate(args.n_obstacles, rng)

    (out / "world.sdf").write_text(world_sdf(start, obstacles))
    (out / "scenario.yaml").write_text(scenario_yaml(args.seed, start, goal, obstacles))
